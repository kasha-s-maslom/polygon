...
using Graph = map<int, vector<pair<int, double>>>; //O(N+M) memory; O(log(N)) для вставок, удаления и поиска по ключу
using Coordinates = map<int, pair<double, double>>; //O(N) память; O(log(N)) для вставок, удаления и поиска по ключу
...
void parseGraph(const string& filename, Graph& graph, Coordinates& coordinates) {
    ...
    auto getId = [&](double lon, double lat) { //O(1) both
        ...
        if (!coordinateToId.count(coord)) { difficulty: //O(log(N))
            coordinateToId[coord] = nodeId; //mem: O(1), diff: O(log(N))
    ...
    while (getline(file, line)) { //diff: O(n)
        ...
        int parentId = getId(lon1, lat1); //mem: O(1), diff: O(n*N) the worst and O(log(n)) median
        ...
        while (getline(ss, child, ';')) { //mem: O(k); diif: O(n), where's k = len(str), n = len of included str
            ...
            if (sscanf(child.c_str(), "%lf,%lf,%lf", &lon2, &lat2, &weight) == 3) { //time: O(1)
                int childId = getId(lon2, lat2); //mem: O(1); diff: O(log(N)+Log(N))
...
int findNearestNode(const Coordinates& coordinates, double lon, double lat) {
    ...
    for (const auto& [node, coord] : coordinates) { //O(N)
...
double printPath(const map<int, int>& parent, int start, int goal, const Graph& graph) {
    ...
    for (int cur = goal; cur != start; cur = parent.at(cur)) { //time: O(N)
...
double DFS(const Graph& graph, int start, int goal) {
    ...
    while (!s.empty()) { // Пока стек не пустой (O(N) в худшем случае)
        ...
        if (visited.count(current)) continue; // Проверка на наличие (O(log(N)))
        
        visited.insert(current); // Помечаем текущую вершину как посещенную (O(log(N)))
        ...
        if (current == goal) { // Проверка на равенство (O(1))
            ...
            printPath(parent, start, goal, graph); // Печатаем путь от start до goal (O(N), где N - длина пути)
            cout << "DFS Time: " << duration_cast<milliseconds>(end_time - start_time).count() << " ms\n"; // Вывод времени выполнения (O(1))
            return printPath(parent, start, goal, graph); // Возвращаем путь (O(N), где N - длина пути)
        }

        // Перебираем соседей текущей вершины
        for (const auto& [neighbor, _] : graph.at(current)) { // Проходим по всем соседям текущей вершины (O(N*Ev), где Ev - количество соседей)
            if (!visited.count(neighbor)) { // Проверяем, не посещен ли сосед (O(log(N)))
                s.push(neighbor); // Добавляем соседа в стек (O(1))
                parent[neighbor] = current; // Устанавливаем текущую вершину как родителя соседа (O(1), но вставка в map O(log(N)))
            }
        }
    }
...
double BFS(const Graph& graph, int start, int goal) {
    ...
    while (!q.empty()) { // Пока очередь не пуста (O(N) в худшем случае)
...
double AStar(const Graph& graph, int start, int goal, const Coordinates& coordinates) {
    ...
    auto heuristic = [&](int node) {
        auto [lon1, lat1] = coordinates.at(node); // Получаем координаты текущего узла (O(log(N)))
        auto [lon2, lat2] = coordinates.at(goal); // Получаем координаты целевого узла (O(log(N)))
    ...
    for (const auto& node : graph) { // Проходим по всем узлам графа (O(V))
        g_costs[node.first] = numeric_limits<double>::infinity(); // Установка g_cost в бесконечность (O(log(N)))
        f_costs[node.first] = numeric_limits<double>::infinity(); // Установка f_cost в бесконечность (O(log(N)))
    } // Общее время: O(V log V)

    g_costs[start] = 0; // Расстояние до стартовой вершины равно 0 (O(log(N)))
    f_costs[start] = heuristic(start); // Вычисляем f_cost для стартовой вершины (O(1))

    pq.emplace(f_costs[start], start); // Добавляем стартовую вершину в приоритетную очередь (O(1) для добавления, O(log(N)) для поддержания порядка)
    ...
    while (!pq.empty()) { // Пока очередь не пуста (в худшем случае O(N^2))
        ...
        pq.pop(); // Удаляем вершину из очереди (O(log(N)))
        ...
        for (const auto& [neighbor, weight] : graph.at(current_node)) { // Проходим по всем соседям текущей вершины (O(Ev), где Ev - количество соседей)
            ...
            if (tentative_g_cost < g_costs[neighbor]) { // Проверяем, меньше ли новое g_cost (O(1))
                g_costs[neighbor] = tentative_g_cost; // Обновляем g_cost до соседа (O(log(N)))
                f_costs[neighbor] = tentative_g_cost + heuristic(neighbor); // Обновляем f_cost для соседа (O(1))
                parent[neighbor] = current_node; // Устанавливаем текущую вершину как родителя соседа (O(1))
                pq.emplace(f_costs[neighbor], neighbor); // Добавляем соседа с обновленным f_cost в приоритетную очередь (O(1) для добавления, O(log(N))для поддержания порядка)
            }
        }
    }
